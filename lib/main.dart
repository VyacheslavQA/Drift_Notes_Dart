...
void main() async {
  ...
  // ✅ ИСПРАВЛЕНИЕ СЧЕТЧИКОВ ЛИМИТОВ (выполняется один раз при запуске)
  await _fixUsageLimitsOnStartup();
...
// ✅ НОВАЯ ФУНКЦИЯ: Исправление счетчиков лимитов при запуске
Future<void> _fixUsageLimitsOnStartup() async {
  try {
    final firebaseService = FirebaseService();

    // Проверяем авторизован ли пользователь
    if (firebaseService.isUserLoggedIn) {
      debugPrint('🔄 Запускаем исправление счетчиков лимитов...');

      await firebaseService.forceRecalculateUsageLimits();

      debugPrint('✅ Счетчики лимитов успешно исправлены при запуске!');
    } else {
      debugPrint('ℹ️ Пользователь не авторизован, пропускаем исправление счетчиков');
    }
  } catch (e) {
    debugPrint('❌ Ошибка исправления счетчиков при запуске: $e');
    // Не блокируем запуск приложения из-за ошибки
  }
}
...
class _DriftNotesAppState extends State<DriftNotesApp>
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) {
      ...
      if (kDebugMode) {
        if (user != null) {
          ...
          // ✅ ИСПРАВЛЯЕМ СЧЕТЧИКИ ПРИ ВХОДЕ ПОЛЬЗОВАТЕЛЯ
          _fixUsageLimitsAfterAuth();
        } else {
  ...
  // ✅ НОВЫЙ МЕТОД: Исправление счетчиков после авторизации
  Future<void> _fixUsageLimitsAfterAuth() async {
    try {
      final firebaseService = FirebaseService();

      debugPrint('🔄 Исправляем счетчики после авторизации...');

      await firebaseService.forceRecalculateUsageLimits();

      debugPrint('✅ Счетчики исправлены после авторизации!');
    } catch (e) {
      debugPrint('❌ Ошибка исправления счетчиков после авторизации: $e');
    }
  }
