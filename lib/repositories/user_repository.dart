...
class UserRepository {
  ...
  // ✅ ИСПРАВЛЕНО: Упрощенное удаление аккаунта пользователя
  Future<void> deleteAccount() async {
    try {
      ...
      // ✅ ИСПРАВЛЕНО: Удаление данных пользователя из Firestore
      try {
        // Удаляем документ пользователя из Firestore
        await _firestore.collection('users').doc(userId).delete();
        debugPrint('✅ Данные пользователя удалены из Firestore');
      } catch (e) {
        debugPrint('⚠️ Ошибка при удалении данных из Firestore: $e');
        // Продолжаем удаление аккаунта даже если не удалось удалить данные
      }

      // ✅ ИСПРАВЛЕНО: Удаление аккаунта из Firebase Auth
      await user.delete();
