// Путь: lib/screens/fishing_diary/edit_fishing_diary_screen.dart
...
class EditFishingDiaryScreen extends StatefulWidget {
  final FishingDiaryModel entry;

  const EditFishingDiaryScreen({super.key, required this.entry});

  @override
  State<EditFishingDiaryScreen> createState() => _EditFishingDiaryScreenState();
}

class _EditFishingDiaryScreenState extends State<EditFishingDiaryScreen> {
  ...
  Future<void> _saveEntry() async {
    ...
    try {
      ...
      await _repository.updateFishingDiaryEntry(updatedEntry);
  ...
  Future<void> _copyEntry() async {
    ...
    try {
      await _repository.copyFishingDiaryEntry(widget.entry.id);
  ...
  Future<void> _deleteEntry() async {
    ...
    if (shouldDelete == true) {
      try {
        await _repository.deleteFishingDiaryEntry(widget.entry.id);
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text(
              localizations.translate('delete_entry').toUpperCase(),
              style: TextStyle(
                fontSize: ResponsiveUtils.getOptimalFontSize(context, 16, maxSize: 18),
                fontWeight: FontWeight.bold,
                color: Colors.white,
