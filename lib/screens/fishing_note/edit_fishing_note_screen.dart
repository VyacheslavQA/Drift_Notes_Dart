...
class _EditFishingNoteScreenState extends State<EditFishingNoteScreen> with SingleTickerProviderStateMixin {
  ...
  // Для хранения карты дневных поклевок
  late Map<String, List<String>> _dayBiteMaps;

  // Для хранения точек рыбалки
  late List<String> _fishingSpots;
  ...
  void initState() {
    ...
    _dayBiteMaps = Map.from(widget.note.dayBiteMaps);
    _fishingSpots = List.from(widget.note.fishingSpots);
  ...
  Future<void> _saveNote() async {
    ...
    try {
      ...
      final updatedNote = widget.note.copyWith(
        ...
        fishingSpots: _fishingSpots,
        dayBiteMaps: _dayBiteMaps,
      );
      ...
      if (isOnline) {
        ...
        if (_newPhotos.isNotEmpty) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Заметка успешно обновлена с новыми фото'),
                backgroundColor: Colors.green,
              ),
            );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Нет подключения к интернету. Изменения будут сохранены локально и отправлены при подключении.'),
  ...
  Widget build(BuildContext context) {
    ...
    foregroundColor: AppConstants.textColor,
      ...
      onPressed: _takePhoto,
                          ...
                          const SizedBox(height: 8),
                          Text(
                            'Существующие фото (${_existingPhotoUrls.length})',
                            style: TextStyle(
                              color: AppConstants.textColor.withOpacity(0.8),
                              fontSize: 14,
                              fontWeight: FontWeight.w500,
                          ...
                          if (_selectedFishingType == 'Карповая рыбалка') ...[
                            ...
                            ElevatedButton.icon(
                              ...
                              label: Text(
                                _mapMarkers.isNotEmpty ? 'Редактировать маркерную карту' : 'Создать маркерную карту',
                                style: TextStyle(
                                  color: AppConstants.textColor,
                                  fontSize: 16,
                                ),
                            ...
                            if (_mapMarkers.isNotEmpty) ...[
                              const SizedBox(height: 8),
                              Text(
                                'Количество маркеров: ${_mapMarkers.length}',
                                style: TextStyle(
                                  color: AppConstants.textColor.withOpacity(0.7),
                                  fontSize: 14,
                                ),
