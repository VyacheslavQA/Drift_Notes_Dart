...
class _FishingNoteDetailScreenState extends State<FishingNoteDetailScreen> {
  ...
  // ОБНОВЛЕННАЯ карточка погоды в едином стиле
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // ВЕРХНЯЯ ЧАСТЬ: иконка + температура + ощущается как
              Row(
                children: [
                  Container(
  ...
  Widget _buildAIAnalysisCard() {
    final localizations = AppLocalizations.of(context);
    final aiData = _note!.aiPrediction;
    if (aiData == null) return const SizedBox();

    // Извлекаем данные из Map
    final overallScore = aiData['overallScore'] as int? ?? 50;
    final recommendation = aiData['recommendation'] as String? ?? '';
    final tips = List<String>.from(aiData['tips'] ?? []);
    final confidencePercent = aiData['confidencePercent'] as int? ?? 50;
              ...
              Row(
                children: [
                  Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: _getScoreColor(overallScore).withValues(alpha: 0.2),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Icon(
                      Icons.psychology,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          '${localizations.translate('ai_bite_forecast')} ($overallScore/100)',
                          style: TextStyle(
                            color: AppConstants.textColor,
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                        ...
                        Text(
                          _getActivityLevelTextFromScore(overallScore, localizations),
                          style: TextStyle(
                            color: _getScoreColor(overallScore),
                            fontSize: 14,
  ...
  // Получение текста уровня активности по скору
  String _getActivityLevelTextFromScore(int score, AppLocalizations localizations) {
