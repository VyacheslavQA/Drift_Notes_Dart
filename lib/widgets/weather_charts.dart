              ...
              LineChartData(
                ...
                lineTouchData: LineTouchData(
                  ...
                  touchTooltipData: LineTouchTooltipData(
                    tooltipBgColor: AppConstants.surfaceColor.withValues(alpha: 0.9),
...
/// Виджет графика активности клева по часам
...
class _BiteActivityChartState extends State<BiteActivityChart> {
  ...
  List<String> timeLabels = [];
  List<FlSpot> sunriseSpots = [];
  List<FlSpot> sunsetSpots = [];
  ...
  void _generateActivityData() {
    ...
    for (int hour = 0; hour < 24; hour++) {
      double activity = _calculateHourlyActivity(hour);
      activitySpots.add(FlSpot(hour.toDouble(), activity));
      timeLabels.add('$hour:00');
    }

    // Добавляем отметки восхода и заката
    if (widget.weatherData.forecast.isNotEmpty) {
      final astro = widget.weatherData.forecast.first.astro;
      final sunriseHour = _parseTimeToHour(astro.sunrise);
      final sunsetHour = _parseTimeToHour(astro.sunset);

      if (sunriseHour != null) {
        sunriseSpots.add(FlSpot(sunriseHour, 1.0));
      }
      if (sunsetHour != null) {
        sunsetSpots.add(FlSpot(sunsetHour, 1.0));
      }
    }
  }
  ...
  double _calculateHourlyActivity(int hour) {
    ...
    // Пиковая активность утром (5-9) и вечером (18-22)
    if (hour >= 5 && hour <= 9) {
      baseActivity = 0.7 + (9 - hour) * 0.05; // Пик в 5-6 утра
    } else if (hour >= 18 && hour <= 22) {
      baseActivity = 0.6 + (hour - 18) * 0.05; // Пик в 20-21 вечера
    } else if (hour >= 10 && hour <= 17) {
      baseActivity = 0.4; // Средняя активность днем
    } else {
      baseActivity = 0.2; // Низкая активность ночью
    }

    // Добавляем небольшую случайную вариацию
    final variation = (math.sin(hour * 0.5) * 0.1);

    return (baseActivity + variation).clamp(0.0, 1.0);
  }
  ...
  Color _getActivityColor(double activity) {
    ...
    if (activity > 0.5) return Colors.lightGreen;
    if (activity > 0.3) return Colors.orange;
    return Colors.red;
  }
            ...
            child: LineChart(
              LineChartData(
                ...
                extraLinesData: ExtraLinesData(
                  verticalLines: [
                    // Восход солнца
                    if (sunriseSpots.isNotEmpty)
                      VerticalLine(
                        x: sunriseSpots.first.x,
                        color: Colors.orange.withValues(alpha: 0.7),
                        strokeWidth: 2,
                        dashArray: [5, 5],
                        label: VerticalLineLabel(
                          show: true,
                          labelResolver: (line) => localizations.translate('sunrise'),
                          style: TextStyle(
                            color: Colors.orange,
                            fontSize: 10,
                            fontWeight: FontWeight.w600,
                    ...
                    // Закат солнца
                    if (sunsetSpots.isNotEmpty)
                      VerticalLine(
                        x: sunsetSpots.first.x,
                        color: Colors.deepPurple.withValues(alpha: 0.7),
                        strokeWidth: 2,
                        dashArray: [5, 5],
                        label: VerticalLineLabel(
                          show: true,
                          labelResolver: (line) => localizations.translate('sunset'),
                          style: TextStyle(
  ...
  Widget _buildActivityLegendItem({
    required Color color,
    required String label,
